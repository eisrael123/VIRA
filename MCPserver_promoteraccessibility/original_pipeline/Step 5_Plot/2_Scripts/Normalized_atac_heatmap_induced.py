...
# INPUTS
# ------------------
bed_path = 'Induced_genes_DG75_BRRF1.txt_Output_main_PEAKS_Matched_TSS_100_TS_50_BRRF1_PEAKS_max_width_16.fraction_max_peaks_0.2.min_single_pos_depth_10.strand_MERGED.txt'
...
for matrix, label in zip([ctl_coverage, brrf1_coverage], ["ctl", "BRRF1"]):
    ...
    plt.yticks([])
    plt.tight_layout()
    plt.savefig(f'{label}_atac_dg75_{length}_BRRF1_test_INDUCED.png', dpi=500)
    plt.close()
...
plt.savefig('summation_curve_combined_atac_dg75_BRRF1_test_INDUCED.png', dpi=500)
plt.close()